...
def test_register_endpoint(test_client):
    with requests_mock.Mocker() as m:
        # Mock token refresh
        m.post("https://api.hubapi.com/oauth/v1/token", json={"access_token": "MOCK_TOKEN", "refresh_token": "MOCK_REFRESH", "expires_in": 3600})
...
def test_pagination_flow(test_client, db_session):
   ...
   # Requesting first page with limit=10
    ...
    assert final_next_after is None, "No more items remain, so next_after should be null"
